...
import "./ERC721I.sol";
import "./IERC721ICallback.sol";

contract ERC721ICallback is ERC721I, IERC721ICallback {
    ...
    function setCallbackInfos(CallbackInfo[] calldata callbackInfos) external onlyOwner {
    ...
    function setOnMintApprovals(address[] calldata onMintApprovals) external onlyOwner {
    ...
    function mintWithCallbacks(
        ...
        if (callbackDatas.length != _callbackInfos.length) {
            revert InvalidCallbackDatasLength(_callbackInfos.length, callbackDatas.length);
        }
        ...
        for (uint256 i = 0; i < _callbackInfos.length; i++) {
            ...
            (bool success, ) = cbInfo.callbackContract.call(
                abi.encodeWithSelector(cbInfo.callbackFunction, msg.sender, tokenIds)
            );

            if (!success) {
                revert CallbackFailed(cbInfo.callbackContract, cbInfo.callbackFunction);
            }
        }
    }
}
